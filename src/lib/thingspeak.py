...
class _ProtocolBase:
    ...
    def send(self, channel: Channel, values: Dict[str, Union[str, int, float]], statusMsg: str) -> int:
...
class _ProtoWeb(_ProtocolBase):
    ...
    _API_PORT = None
    _HTTP_POST_TEMPLATE = "GET /update?api_key={write_api_key}&{data}&status={statusMsg} HTTP/1.1\r\n" \
                          "Host: {host}\r\n" \
                          "Connection: close\r\n\r\n"
    ...
    @staticmethod
    def _make_http_data(channel: Channel,
        ...
        return _ProtoWeb._HTTP_POST_TEMPLATE.format(
            host=_ProtoWeb._API_HOST,
            write_api_key=channel.write_key,
    ...
    def send(self,
        ...
        reply = self._parse_reply(
            self._send_to_socket(
                sock, self._make_http_data(channel, values, statusMsg)
            )
        )
...
class ThingSpeakAPI:
    ...
    def send(self,
        ...
        messages_count = self._api.send(self._get_channel(channel_name),
                                        values,
                                        statusMsg
                                        )
